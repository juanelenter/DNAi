...
#Loads
test<-foreach(i = 1 : n_it, .options.RNG = 100) %do% {
    t(read.csv(paste('../output/sim/',i,'_', 'idx_train.csv', sep=""), header=FALSE,  colClasses = 'integer'))
}
#print(dim(test[[1]]))
train<-foreach(i = 1 : n_it, .options.RNG = 100) %do% {
    t(read.csv(paste('../output/sim/',i,'_', 'idx_test.csv', sep=""), header=FALSE,  colClasses = 'integer'))
}
...
foreach(i = 1 : n_it, .options.RNG = 100) %dorng% {
    ...
    ridge.pred <- predict(lasso.mod, Xtest_m)
