        ...
        if self.neptune:
            if fing:
                neptune.init("dna-i/"+project_name,
                             api_token='eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vdWkubmVwdHVuZS5haSIsImFwaV91cmwiOiJodHRwczovL3VpLm5lcHR1bmUuYWkiLCJhcGlfa2V5IjoiMWYzMzhjMjItYjczNC00NzZhLWFlZTYtOTI2NzE5MzUwZmNkIn0=', proxies= {'http':"http://httpproxy.fing.edu.uy:3128/",
    ...
    def add_params_torch(self, torch_model, step=None ):
        ...
        if self.comet:
            for name, param in torch_model.named_parameters():                    
                self.comet_experiment.log_histogram_3d(param.detach().cpu().numpy().tolist(),name=name, step=step)
