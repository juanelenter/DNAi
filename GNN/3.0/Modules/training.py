...
class Trainer:
    ...
    def __init__(self, model, data, nEpochs, batchSize, **kwargs):
        ...
        if 'trial' in kwargs.keys():
            ...
            trial = None
        # COMET STUFF
        if 'cometLogger' in kwargs.keys():
            comet_logger = kwargs['cometLogger']
        else:
            comet_logger = None
        ...
        self.trainingOptions['trial'] = trial
        self.trainingOptions['cometLogger'] = comet_logger
    ...
    def validationStep(self, epoch, nBatches, batch):
        ...
        with torch.no_grad():
            ...
            lossValueValid = 0
            costValid = 0
            yHatValid = torch.empty(yValid.shape).to(self.model.device)
            for i, x in enumerate(xValid):
                x = x.reshape(1,x.shape[0],x.shape[1]) #sino me chillaba .loss()
                
                yHatValid[i] = self.model.archit(x)

                # Compute loss
                lossValueValid += self.model.loss(yHatValid[i], yValid[i]).item()
                # Compute accuracy:
                costValid += self.data.evaluate(yHatValid[i], yValid[i])
            costValid /= yValid.shape[0]
            lossValueValid /= yValid.shape[0]
            r2Valid = r2_score(yValid.squeeze().cpu(), yHatValid.squeeze().cpu())
            # Finish measuring time
            endTime = datetime.datetime.now()

            timeElapsed = abs(endTime - startTime).total_seconds()

            # get comet logger if available
            comet_logger = self.trainingOptions['cometLogger']

            if comet_logger is not None:
                comet_logger.log_metrics('costValid', costValid, epoch=epoch * nBatches + 1)
                comet_logger.log_metrics('lossValid', lossValueValid, epoch=epoch * nBatches + 1)
                comet_logger.log_metrics('r2Valid', r2Valid, epoch=epoch * nBatches + 1)
                comet_logger.add_params_torch(self.model.archit, step=epoch * nBatches + 1)
                
            if self.trainingOptions['trial']:
                self.trainingOptions['trial'].report(lossValueValid, epoch * nBatches + 1)
                    # Handle pruning based on the intermediate value.
                if self.trainingOptions['trial'].should_prune():
                    raise optuna.exceptions.TrialPruned()

            print(f"-------- Validation r2 score = {r2Valid}.")
        return lossValueValid, costValid, timeElapsed
    ...
    def train(self):
        ...
        while epoch < nEpochs \
            ...
            if doLearningRateDecay:
                ...
                if False:#doPrint: ESTABA TIRANDO ERROR
            ...
            while batch < nBatches \
                ...
                if (epoch * nBatches + batch) % validationInterval == 0:
                    ...
                    lossValueValid, costValueValid, timeElapsed = \
                                                           self.validationStep(epoch, nBatches, batch)
