...
def evaluate(model, data, **kwargs):
    ...
    with torch.no_grad():
        for i, x in enumerate(xTest):
            ...
            yHatTest = model.archit(x)

            # Compute loss
            lossBest += model.loss(yHatTest, yTest[i].reshape(yHatTest.shape)).item()

            # Compute accuracy:
            costBest += data.evaluate(yHatTest, yTest[i].reshape(yHatTest.shape))
        costBest /= yHatTest.shape[0]
        lossBest /= yHatTest.shape[0]
    ...
    with torch.no_grad():
        for i, x in enumerate(xTest):
            ...
            # Compute loss
            lossLast += model.loss(yHatTest, yTest[i].reshape(yHatTest.shape)).item()

            # Compute accuracy:
            costLast += data.evaluate(yHatTest, yTest[i].reshape(yHatTest.shape))
        costLast /= yHatTest.shape[0]
        lossLast /= yHatTest.shape[0]
