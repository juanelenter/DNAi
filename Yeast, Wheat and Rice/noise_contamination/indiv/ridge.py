...
alphas = [200, 1500, 1350, 1700, 2250, 2750, 1000, 1000, 1000, 1250, 2750, 2750]
#%%
i = 0
M = 10
...
for name, y in pheno_12.iteritems():
    ...
    for del_ratio in del_ratios:
        ...
        for k in np.arange(M):
            ...
            rid.fit(X_train, y_train_std)
            
            rid_predictions = rid.predict(X_test)
            r2 = r2_score(y_test_std, rid_predictions)
    ...
    clear_output()
    with open('results/ridge/r2_ridge_drop_samples_{}.pickle'.format(name), 'wb') as f:
        pickle.dump(r2s, f)
