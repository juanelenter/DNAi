...
def bayes_a(X, y, idx_train, idx_test, normalize_pheno = False, normalize_geno = False, niter=30000, R2 = 0.3):
    r.assign('X',X)
    r.assign('y',y)
...
def train_test(X_train, Y_train, X_test, Y_test, cv_params, custom_grid = False):
    ...
    train_predictions, test_predictions = bayes_a(X, y, idx_train, idx_test, normalize_pheno = False, normalize_geno = False)
    #metrics
    if custom_grid:
