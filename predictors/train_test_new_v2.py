...
def train_test(cfg_name, model_name, opt_args):
    ...
    for split_indx in split_indxs: 
        ...
        if opt_args["debug"]:
            split.debug_mode()
        splits.append(split)
    shuffle(splits) # shuffle so that gridsearch split is different each run
    ...
    if model_name != "all": #train one model only
        ...
        for i, split in zip(split_indxs, splits):
            ...
            grid_params, model_params, train_preds, test_preds, metrics, model_inst = model.train_test(split.X_train, split.Y_train,
                                                                                               split.X_test, split.Y_test,
                                                                                               cv_params, custom_grid)
            #if log_experiment:
                #logger.add_params(grid_params)
            splits_c = splits.copy()
            del(splits_c[i])
            r_mean = metrics["r_test"][0]
            for split_eval in splits_c:
                _, _, metrics_eval = fit_test(model_inst, grid_params, split_eval.X_train, split_eval.Y_train,
                                                            split_eval.X_test, split_eval.Y_test)
                r_mean += metrics_eval["r_test"][0]
            r_mean /= len(splits)
            r_means_new.append(r_mean)
            if r_mean > best_r_mean:
                best_r_mean = r_mean
                best_grid_params = grid_params
        ...
        for dict_params in model_params_list:
            :gfor key, val in dict_params.items():
            :g	try:
            :g		final_dict[key].append(val)
            :g	except:
            :g		final_dict[key] = [val]
        if log_experiment:            
